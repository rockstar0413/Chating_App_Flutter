...
Future<String> userLogin(number, password) async {
  ...
  // request.add(utf8.encode(json.encode({"number": number, "password": password})));
  // HttpClientResponse response = await request.close();
  //  responseData = await response.transform(utf8.decoder).join();
  final response = await client.post(
    Uri.parse(Constants.SERVER_URL + url),
    // Uri.parse("http://10.10.11.143:4000/api/v1/auth/loginV2"),
    headers: {HttpHeaders.contentTypeHeader: 'application/json'},
    body: json.encode({"email": number, "password": password}),
  );
  print(response.body);
  if (response.statusCode == 200) {
    responseData = response.body;
...
Future<String> getnumbers(int index) async {
  ...
  final response = await client.post(
    Uri.parse(Constants.SERVER_URL + "v1/numbers"),
    headers: {HttpHeaders.contentTypeHeader: 'application/json',
      'utoken':deviceId
    },
