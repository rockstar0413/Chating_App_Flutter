...
Future<String> sendnotification(String msg, String title, String tokeen) async {
  var responseData;
  final String url =
      "https://localtalk.mobi/communicator/api/notifications/push";
  ...
  final response = await client.post(
    ...
    body: json.encode({
      "pushData": {"message": msg, "title": title, "deviceToken": tokeen}
    }),
  );
...
Future<String> deleteit(String token, messageId) async {
  ...
  log(token);
  final String url =
      "https://localtalk.mobi/communicator/api/v1/message/deleteMessageEveryone";
  final client = new http.Client();
  final response = await client.post(
    Uri.parse(url),
...
Future<String> uodatecount(String cid, String count, String token) async {
  ...
  log(token);
  final String url =
      "https://localtalk.mobi/communicator/api/v1/conversions/updateChatCount";
  final client = new http.Client();
  final response = await client.post(
    Uri.parse(url),
    headers: {HttpHeaders.AUTHORIZATION: 'Bearer ' + token},
...
Future<String> createRoom(String id, String token, String two) async {
  ...
  final response = await client.post(
    ...
    body: json
        .encode({"user_one": id, "user_two": two, "lastMessage": 'Hi, there'}),
  );
  if (response.statusCode == 200) {
    responseData = response.body;
  } else {
...
Future<String> setDislikePost(
    String posterId, String token, String userId) async {
  var responseData;
