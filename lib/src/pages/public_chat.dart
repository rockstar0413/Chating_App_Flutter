...
class _PublicChatWidgetState extends StateMVC<PublicChatWidget> {
  ...
  void initState() {
    ...
    timer = Timer.periodic(
        Duration(minutes: _con.refresh_location_time_interval ?? 3),
        (Timer t) => _onLoadMore());
  ...
  setCurrentLocation() async {
    Geolocator.getCurrentPosition(desiredAccuracy: LocationAccuracy.best)
      ...
      var myLocation = _currentPosition.latitude.toString() +
          "," +
              ...
              child: SmartRefresher(
                ...
                child: ListView.builder(
                  ...
                  itemBuilder: (context, i) {
                    var itemList = [];
                    if (json
                            .decode(json.encode(_listMessages[i].imgs))
                            .length !=
                        0) {
                      for (var item
                          in json.decode(json.encode(_listMessages[i].imgs))) {
                        if (item.toString().contains("min")) {
                                        ...
                                        builder: (context) {
                                          return AlertDialog(
                                            ...
                                            actions: <Widget>[
                                              GestureDetector(
                                                onTap: () {
                                                  if (checklisttop.length <
                                                      _con.top_message_num) {
                                                    _con.topItUp(
                                                        _listMessages[i]
                                                            .messageId
                                                            .toString());
                  ...
                  GestureDetector(
                      onTap: () {
                        if (chatcontroller.text.trim().length == 0) {
                          Fluttertoast.showToast(msg: 'input'.tr());
                        } else {
                          if (!isMessageSent) {
                            _sendMessage();
  ...
  getAllowPopUpShared() async {
    final sharedPreferences = await SharedPreferences.getInstance();
    setState(() {
      ifPopupClose = sharedPreferences.getBool('allow_app_popup_close');
    });
    if (ifPopupClose == null) {
      setState(() {
        ifPopupClose = false;
      });
    }
  ...
  allowAppDialog() {
    return showDialog(
        ...
        builder: (BuildContext context) {
          return Dialog(
            backgroundColor: Color(0xfff6f5d1),
            elevation: 0,
            shape: RoundedRectangleBorder(
                borderRadius: BorderRadius.circular(20.0),
                side: BorderSide(width: 1.5, color: Color(0xff495894))),
            child: Container(
              height: config.App(context).appHeight(44),
              child: Padding(
                      ...
                      child: ElevatedButton(
                        style: ElevatedButton.styleFrom(
                            ...
                            shape: RoundedRectangleBorder(
                                ...
                                side: BorderSide(
                                    color: Color(0xff495894), width: 1))),
                        onPressed: () {
                      ...
                      child: ElevatedButton(
                        ...
                        onPressed: () async {
                          ...
                          sharedPreferences.setBool(
                              'allow_app_popup_close', true);
                          Navigator.of(context).pop();
                        },
                        child: Text(
                          "close_and_notshow".tr(),
                          textAlign: TextAlign.center,
                          style: TextStyle(
                            fontSize: 9.5,
                            color: Colors.white,
                            ...
                            Container(
                              ...
                              child: Row(
                                children: [
                                  ...
                                  Text(
                                      double.parse(_listMessages[i].distance)
                                              .toStringAsFixed(0)
                                              .toString() +
